...
export default function MyBookingsPage() {
  ...
  const isPastModificationCutoff = (checkInDate) => {
    if (!checkInDate) return false;

    const [year, month, day] = checkInDate.split("-").map(Number);
    const now = new Date();

    const cutoff = new Date(year, month - 1, day, 18, 0, 0, 0);
  ...
  const handleOpenModifyModal = async (booking) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRoomBookings(
          data.filter(
            (b) => (b.id || b.booking_id) !== currentBookingId,
          ),
        );
      }
    } catch (err) {
  ...
  const excludedIntervals = useMemo(() => {
    return roomBookings
      ...
      .map((b) => {
        const start = parseStringToLocalDate(
          b.check_in_date || b.check_in,
        );

        const end = parseStringToLocalDate(
          b.check_out_date || b.check_out,
        );

        const dayBeforeCheckOut = new Date(end);
        dayBeforeCheckOut.setDate(
          dayBeforeCheckOut.getDate() - 1,
        );

        return {
          start,
          end:
            dayBeforeCheckOut >= start
              ? dayBeforeCheckOut
              : start,
        };
      })
  ...
  const handleCancelBooking = async (bookingId) => {
    const booking = bookings.find(
      (b) => (b.id || b.booking_id) === bookingId,
    );

    const checkIn =
      booking?.check_in_date || booking?.check_in;

    if (isPastModificationCutoff(checkIn)) {
      alert(
        "This reservation can no longer be cancelled online. Please contact the hotel for assistance.",
      );
      return;
    }
  ...
  const handleModifySubmit = async (e) => {
    ...
    if (
      isPastModificationCutoff(
        selectedBooking?.check_in_date ||
          selectedBooking?.check_in,
      )
    ) {
      setModifyError(
        "This reservation can no longer be modified online. Please contact the hotel for assistance.",
      );
      return;
    }
    ...
    const token = localStorage.getItem("accessToken");
    const bookingId =
      selectedBooking.id || selectedBooking.booking_id;
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            ...
            <p className="text-white-50 mb-0">
              Welcome back
              {user?.first_name ? `, ${user.first_name}` : ""}! Here
        ...
        {loading ? (
          ...
          <div className="row g-4">
            {bookings.map((booking) => {
              ...
              const cutoffPassed =
                isPastModificationCutoff(checkIn);
                            ...
                            <div className="text-muted small fst-italic">
                              {status === "Cancelled"
                                ...
                                : status === "Checked Out" ||
                                  ...
                                  : "Online changes are no longer available"}
                            </div>
